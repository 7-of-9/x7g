...
#include <D3DX8.h>
#include "../../resource.h"
#include "../include/d3dfont_custom.h"
...
HRESULT CD3DFont::DrawTextScaledDM( FLOAT x, FLOAT y, FLOAT z,
                                    FLOAT fXScale, FLOAT fYScale, DWORD dwColor,
                                    TCHAR* strText, DWORD dwFlags, D3DVIEWPORT8* pVP)
{
    if( m_pd3dDevice == NULL )
        return E_FAIL;
    ...
    FLOAT sx  = (x+1.0f)*pVP->Width/2;
    FLOAT sy  = (y+1.0f)*pVP->Height/2;
    FLOAT sz  = z;
    FLOAT rhw = 1.0f;
    FLOAT fStartX = sx;
    ...
    while( *strText )
    {
        ...
        w *= (fXScale*pVP->Width)/fLineHeight;
        h *= (fYScale*pVP->Height)/fLineHeight;
...
void CD3DFont::SetRenderStateDM()
{
    // Set up renderstate
    m_pd3dDevice->CaptureStateBlock( m_dwSavedStateBlock );
    m_pd3dDevice->ApplyStateBlock( m_dwDrawTextStateBlock );
...
void CD3DFont::ResetRenderStateDM()
{
    // Restore the modified renderstates
    m_pd3dDevice->ApplyStateBlock( m_dwSavedStateBlock );
}
