...
#include <tchar.h>
#include <D3D8.h>
#include "../include/d3dapp_custom.h"
...
// Dom's additions
	m_bVSync			= FALSE ;
	m_nFSAA				= 0 ;			// (none)
	m_nFullScreenWidth  = 640 ;
	m_nFullScreenHeight = 480 ;
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
  ...
  WS_EX_CONTEXTHELP|
  WS_EX_ACCEPTFILES,
  "D3D Window",		    									 // pointer to registered class name
  m_strWindowTitle,											 // pointer to window name
  m_dwWindowStyle,   										 // window style
  m_dwCreationX,  										     // horizontal position of window
  m_dwCreationY,  										     // vertical position of window
  (rc.right-rc.left),										 // window width
  (rc.bottom-rc.top),          								 // window height
  0L,      													 // handle to parent or owner window
  NULL/*LoadMenu( hInstance, MAKEINTRESOURCE(IDR_MENU) )*/,  // handle to menu, or child-window identifier
  hInstance,  												 // handle to application instance
  0L														 // pointer to window-creation data
							   );					
                                  ...
                                  LPARAM lParam )
{
    ...
    switch( uMsg )
    {
        ...
        case WM_ACTIVATEAPP:
			...
			{
				SetKey( VK_NUMLOCK, m_bKS[ VK_NUMLOCK ] );
				SetKey( VK_CAPITAL, m_bKS[ VK_CAPITAL ] );
				SetKey( VK_SCROLL,  m_bKS[ VK_SCROLL  ] );
			}
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	// DM
	if( m_nFSAA != 0 )
		m_d3dpp.MultiSampleType	   = (D3DMULTISAMPLE_TYPE)m_nFSAA ;
    ...
    // Create the device
    hr = m_pD3D->CreateDevice( m_dwAdapter,
...
HRESULT CD3DApplication::ToggleFullscreen()
{
	...
	// DM
	for( DWORD m = 0 ; m < pDeviceInfo->dwNumModes ; m++ )
	{
		if( pDeviceInfo->modes[m].Width == m_nFullScreenWidth &&
			pDeviceInfo->modes[m].Height == m_nFullScreenHeight &&
			(pDeviceInfo->modes[m].Format == D3DFMT_A8R8G8B8 || 
			 pDeviceInfo->modes[m].Format == D3DFMT_X8R8G8B8) )
		{
	        pModeInfo = &pDeviceInfo->modes[m];
			break ;
		}
	}
...
HRESULT CD3DApplication::Render3DEnvironment()
{
    ...
    // Keep track of the frame count
    {
		...
		// DM
        if( fTime - fLastTime > 0.05f )	// twenty times per second!
        {
			...
			// DM
			MultiSampleType = m_d3dpp.MultiSampleType ;
